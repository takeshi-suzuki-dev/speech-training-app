...
このテーブルは Phase 3 のための土台である。アプリケーションはこのテーブルを読みも書きもせず、
文との同期も取っていない。サンプル音声は `sentence_template_audios` から提供されており、
そちらが自身の `voice_id` と `model_id` を持っている。このテーブルが存在する理由である複数音声機能は
Phase 3 に延期されている。

したがって、意図的に空のままにしている。過去に一度 `sentence_template_audios` を1行ずつ引き写す形で
データを投入したが、その時点で陳腐化が始まった。以後に作成された文には行が作られず、アプリ経由で
作成された行は `model_id` が null のままだった。どのコードも維持しないデータは正しさを保てない。
中途半端に正しいテーブルは空のテーブルより悪い。読み手に「信用してよい」と誤解させるからである。

Phase 3 で機能を実装する際に、維持するコードと一緒にデータを投入する。すなわち、文の作成時に行を作り、
文の削除時に論理削除する。それまでは空であることが正直な状態であり、
`sentence_template_audios.voice_option_id` が null のままであるのも同じ理由による。
