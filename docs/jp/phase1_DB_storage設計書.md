...
見本音声のパスは `sentence_template_audios.audio_path` で管理する。固定定型文と2つの固定音声だけであれば、テンプレートIDと voice_name で見本音声ファイルを一意に特定できる。
...
2. Backend が `voice_name` が `roger` であることを検証する。
